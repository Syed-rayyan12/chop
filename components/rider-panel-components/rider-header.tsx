...
export function RiderHeader({ collapsed, setCollapsed,notifications }: DashboardHeaderProps) {
  ...
  return (
    <header className="bg-background border-b border-primary/50 px-4 h-16 py-[13px]">
      <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <div className="hidden lg:block">
              <Button
                ...
                className="text-priamry hover:text-secondary cursor-pointer hover:bg-secondary/10"
              >
                <MenuIcon className="h-4 w-4" />
              </Button>
            </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="relative hidden sm:block">
            <Search className="absolute left-3 top-1/2 transform text-primary -translate-y-1/2  h-4 w-4" />
            <Input
              placeholder="Search orders..."
              className="pl-10 w-80 border-primary/50"
            />
          </div>
          {/* Notifications */}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button  variant="mys" size="sm" className="relative cursor-pointer text-foreground/60">
            ...
            <DropdownMenuContent align="end" className="w-80 border border-primary/50 bg-white">
              {notifications.length === 0 ? (
                <DropdownMenuItem className="p-3 text-sm text-foreground">
                  No new notifications
                </DropdownMenuItem>
              ) : (
                notifications.slice(0, 3).map((notification) => (
                  <DropdownMenuItem key={notification.id} className="p-3 hover:bg-primary text-foreground  hover:text-white">
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-8 w-8 rounded-full hover:bg-transparent">
                <Avatar className="h-8 w-8">
                  <AvatarFallback className=" text-white bg-primary cursor-pointer">
                    {user
